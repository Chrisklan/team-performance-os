-- =============================================================================
-- 20260919000018_assign_coach_email.sql — Trainer-Login coach@c-klan.de (AP-29)
--
-- Problem:
--   Das Seed-Profil von Thomas Mueller (Rolle coach) traegt thomas.mueller@tpos.local. ".local" ist
...
--     wird von thomas.mueller@tpos.local auf coach@c-klan.de umgestellt, damit der
...
--   matcht der erste Magic-Link-Login mit coach@c-klan.de genau dieses Profil;
...
DO $$
DECLARE
  c_profile_id  constant uuid := 'a0000000-0000-0000-0000-000000000019';
  c_old_email   constant text := 'thomas.mueller@tpos.local';
  c_new_email   constant text := 'coach@c-klan.de';
  v_current     text;
  v_updated     integer;
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'TPOS coach email: seed profile % not found; refusing to assign %', c_profile_id, c_new_email
      USING ERRCODE = 'P0002';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'TPOS coach email: e-mail % is already assigned to a different profile', c_new_email
      USING ERRCODE = '23505';
  END IF;

  -- (3) Bereits umgestellt: nichts tun.
  IF v_current = c_new_email THEN
    RAISE NOTICE 'TPOS coach email: seed profile % already uses %; no change', c_profile_id, c_new_email;
    RETURN;
  END IF;

  -- (4) Nur von exakt der erwarteten alten Adresse umstellen.
  IF v_current IS DISTINCT FROM c_old_email THEN
    RAISE EXCEPTION 'TPOS coach email: seed profile % has unexpected e-mail % (expected %)', c_profile_id, coalesce(v_current, '<null>'), c_old_email
      USING ERRCODE = '22023';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'TPOS coach email: auth user for % already exists; refusing to reassign profile %', c_new_email, c_profile_id
      USING ERRCODE = '23505';
  END IF;
  ...
  IF v_updated <> 1 THEN
    RAISE EXCEPTION 'TPOS coach email: expected to update exactly 1 profile, updated %', v_updated;
  END IF;
END $$;
...
--       FROM public.profiles pr
--       WHERE pr.id = 'a0000000-0000-0000-0000-000000000019';
--     Erwartet: email = coach@c-klan.de,
--               role = coach, full_name = Thomas Müller
--
-- V2  Genau ein Profil mit der Zieladresse, keins mehr mit der alten:
--       SELECT lower(btrim(email)) AS email, count(*)
--       FROM public.profiles
--       WHERE lower(btrim(email)) IN ('coach@c-klan.de', 'thomas.mueller@tpos.local')
--       GROUP BY 1;
--     Erwartet: 1 Zeile, coach@c-klan.de | 1
--
-- V3  app.persons weiterhin an die Seed-ID gebunden (vor dem ersten Login):
--       SELECT pe.id, pe.auth_user_id
--       FROM app.persons pe
--       WHERE pe.id = 'a0000000-0000-0000-0000-000000000019';
--     Erwartet: auth_user_id = a0000000-0000-0000-0000-000000000019
-- =============================================================================
