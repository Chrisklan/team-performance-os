...
--   5. Personenzeile anonymisieren (Crypto Shredding, die id bleibt), samt
--      Darstellungspraeferenz der Body Map (AP-43).
...
--
-- AP-43 (2026-09-21): Schritt 5 setzt zusaetzlich persons.body_map_figure auf die
-- Vorgabe zurueck. Der Tippunkt und das Feld svg aus der Body Map brauchten
-- keine Aenderung: Schritt 2 loescht die ganze Check-In Zeile und Schritt 6
-- adressiert das audit_log ueber person_id im jsonb, nicht ueber eine
-- Feldliste. Der Test 14_shred_person.pgtap.sql belegt das mit einem echten
-- Tippunkt, statt es anzunehmen.
--
-- Voraussetzung: 08_reconciling.sql, 09_rpcs.sql, 17_squad_figure.sql. Idempotent.
-- Tests: backend/14_shred_person.pgtap.sql.
-- =============================================================================
